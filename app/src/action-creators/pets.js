...
export const addPet = (pet, history) => async (dispatch, getState) => {
  const headers = { 'Content-Type': 'application/json' }
  const method = 'POST'
  const body = JSON.stringify(pet)

  const result = await fetch(`${url}/pets`, {
    headers,
    method,
    body
  }).then(res => res.json())
...
export const chgPet = (field, value) => (dispatch, getState) => {
  dispatch({ type: CHG_CURRENT_PET, payload: { [field]: value } })
}
